...
(setq load-path (cons (substitute-in-file-name "${DAIKONDIR}/emacs/")
                      load-path))
...
Remake it first if it is more than a week old."
  (interactive)
  (let* ((tags-file (substitute-in-file-name "$DAIKONDIR/java/TAGS"))
    ...
    (if (or (not tags-file-exists)
        ...
        (let ((default-directory (substitute-in-file-name "$DAIKONDIR/java/"))
...
(defun daikon-info (&optional basename)
  ...
  (let* ((dir (substitute-in-file-name "$DAIKONDIR/doc/"))
...
Does nothing if a compilation is already running unless `force' is non-nil."
  ...
  (if (or force (not (compilation-is-running)))
      (let ((default-directory (substitute-in-file-name "$DAIKONDIR/doc/")))
