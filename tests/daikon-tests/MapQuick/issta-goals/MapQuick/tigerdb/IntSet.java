...
public class IntSet
{
  ...
  public void checkRep()
  {
    for (int n=0; n<=1; n++) {
      ...
      Assert.assert("0 <= span[n] <= elts[n].length", (0 <= size) && (size <= nums.length));

      for (int i=1; i<size; i++) {
        Assert.assert("The elements of elts[n] up to index span[n] are unique", nums[i-1] != nums[i]);
        Assert.assert("The elements of elts[n] up to index span[n] are sorted", nums[i-1] <= nums[i]);
      }
  ...
  public IntSet union(IntSet other)
  {
    ...
    // Assert.assert((this.size() + other.size()) == (result.size() + overlap));
