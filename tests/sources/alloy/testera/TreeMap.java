...
//TESTERA package java.util;
package alloy.testera; //TESTERA
import java.util.*; //TESTERA
	             ...
	             implements SortedMap, Cloneable, java.io.Serializable
{
    ...
    //TESTERA private transient Entry root = null;
    transient Entry root = null; //TESTERA
	...
	//DAIKON private transient Set entrySet = new EntrySetView();
	Set entrySet = null; //DAIKON

	public Set entrySet() {
	    return entrySet;
	}

	//DAIKON private class EntrySetView extends AbstractSet {
	//DAIKON    private transient int size = -1, sizeModCount;

	//DAIKON    public int size() {
	//DAIKON	if (size == -1 || sizeModCount != TreeMap.this.modCount) {
	//DAIKON	    size = 0;  sizeModCount = TreeMap.this.modCount;
	//DAIKON	    java.util.Iterator i = iterator();
	//DAIKON	    while (i.hasNext()) {
	//DAIKON		size++;
	//DAIKON		i.next();
	//DAIKON	    }
	//DAIKON	}
	...
	//DAIKON    public boolean isEmpty() {
	//DAIKON	return !iterator().hasNext();
	//DAIKON    }

	//DAIKON    public boolean contains(Object o) {
	//DAIKON	if (!(o instanceof Map.Entry))
	//DAIKON	    return false;
	//DAIKON	Map.Entry entry = (Map.Entry)o;
	//DAIKON	Object key = entry.getKey();
        //DAIKON        if (!inRange(key))
	//DAIKON                   return false;
        //DAIKON        TreeMap.Entry node = getEntry(key);
	//DAIKON               return node != null &&
        //DAIKON               valEquals(node.getValue(), entry.getValue());
	//DAIKON    }

	//DAIKON    public boolean remove(Object o) {
	//DAIKON	if (!(o instanceof Map.Entry))
	//DAIKON	    return false;
	//DAIKON	Map.Entry entry = (Map.Entry)o;
	//DAIKON	Object key = entry.getKey();
	...
	//DAIKON                   return false;
        //DAIKON        TreeMap.Entry node = getEntry(key);
	//DAIKON               if (node!=null && valEquals(node.getValue(),entry.getValue())){
	//DAIKON	    deleteEntry(node);
	//DAIKON	    return true;
        //DAIKON        }
	//DAIKON        return false;
	//DAIKON    }

	//DAIKON    public java.util.Iterator iterator() {
	//DAIKON	return new Iterator(
	//DAIKON                   (fromStart ? firstEntry() : getCeilEntry(fromKey)),
        //DAIKON            (toEnd     ? null	      : getCeilEntry(toKey)));
	//DAIKON    }
	//DAIKON }
     ...
     */
    //TESTERA private void deleteEntry(Entry p) {
