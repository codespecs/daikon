...
public final class VarInfo implements Cloneable, Serializable {
	...
	private Object po_higher;
	// either null, VarInfo, or VarInfo[] with no duplicates
	private Object po_lower;
	// either null, VarInfo, or VarInfo[] with no duplicates
	private int[] po_higher_nonce;
	// null iff po_higher is null, else length == po_higher().size()
				...
				if (lower.missingOutOfBounds()) {
					debugMissing.fine(
						"Var "
							...
							+ " "
							+ lower.name.name()
							+ " out of bounds implies "
							+ ppt.name()
							+ " "
							+ name.name()
							+ " out of bounds. ");
					derived.missing_array_bounds = true;
					// don't force us to recalc this
					return (true);
				}
				return (false);
			}
	 ...
	 **/
	public void addHigherPO(VarInfo higher, int nonce) {
		VarInfo lower = this;
		...
		if (Dataflow.debugInit.isLoggable(Level.FINE))
			Dataflow.debugInit.fine(
				"addHigherPO " + higher.name.name() + " to " + name.name());
		...
		Assert.assertTrue(
			...
			"lower.type == higher.type");
		Assert.assertTrue(
			lower.rep_type == higher.rep_type,
			"lower.rep_type == higher.rep_type");
		Assert.assertTrue(
			lower.file_rep_type == higher.file_rep_type,
			"lower.file_rep_type == higher.file_rep_type");
		...
		if (lower.po_higher == null) {
			...
			lower.po_higher =
				new VarInfo[] {(VarInfo) lower.po_higher, higher };
			lower.po_higher_nonce =
				new int[] { lower.po_higher_nonce[0], nonce };
		} else {
	...
	boolean isSimilarVarInfo(VarInfo other) {
		...
		Assert.assertTrue(
			file_rep_type.equals(other.file_rep_type),
	...
	/**
		...
		*/
	//added by Chen for use in the simple incremental algorithm (6/22/04)
	...
	/**
	  ...
	  */
	//added by Chen for use in the simple incremental algorithm (6/23/04)
