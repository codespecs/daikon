...
public final class TypeStack {
  ...
  private final Map<InstructionHandle, OperandStack> stackMap =
      new HashMap<InstructionHandle, OperandStack>();
  private final Map<InstructionHandle, InstructionHandle> parentMap =
  ...
  private void createMap(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/ final InstructionList l,
      final CodeExceptionGen[] exceptionTable) {
    if (l == null) {
      // return;
  ...
  private boolean initParents(
    ...
    if (prev == null
        || !(prev.getInstruction()
            instanceof GotoInstruction)) // || prev.getInstruction() instanceof ATHROW))
    {
  ...
  private boolean inChain(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/ final InstructionHandle h) {
    return inChainHelper(parentMap.get(h), h);
  }

  /*@EnsuresNonNullIf(result=true, expression="#2")*/
  private boolean inChainHelper(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/
  ...
  private void initStack(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/ InstructionHandle hand) {
    ...
    if (inst instanceof ACONST_NULL) {
      ...
      if (retType != Type.VOID) stack.push(retType);
  ...
  private void handleMath(
    ...
    if (inst instanceof DADD) {
      ...
      throw new RuntimeException("Unknown arithmetic instruction type: " + inst);
    }
  }
  ...
  private void handleArray(
    ...
    if (inst instanceof AALOAD) {
      ...
      throw new RuntimeException("Unknown array instruction type: " + inst);
    }
  }

  /*@RequiresNonNull("stack")*/
  // is this right, or too strict?
  private void handleBranch(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/ BranchInstruction inst) {
    if (inst instanceof GotoInstruction) {
      if (inst instanceof GOTO) {
        ...
        throw new RuntimeException("Unknown goto instruction type: " + inst);
      }
      ...
      if (inst instanceof IF_ACMPEQ) {
        ...
        throw new RuntimeException("Unknown if instruction type: " + inst);
      }
      ...
      if (inst instanceof LOOKUPSWITCH) {
        ...
        throw new RuntimeException("Unknown select instruction type: " + inst);
      }
    } else {
      throw new RuntimeException("Unknown branching instruction type: " + inst);
    }
  }

  /*@RequiresNonNull("stack")*/
  private void handleConv(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/ ConversionInstruction inst) {
    if (inst instanceof D2F) {
      ...
      throw new RuntimeException("Unknown conversion instruction type: " + inst);
    }
  }
  ...
  private void handleCP(/*>>>@UnknownInitialization @Raw TypeStack this,*/ CPInstruction inst) {
    if (inst instanceof ANEWARRAY) {
      ...
      if (inst instanceof GETFIELD) {
        ...
        if (ret != Type.VOID) stack.push(ret);
        ...
        throw new RuntimeException("Unknown field or method instruction type: " + inst);
      }
  ...
  private void handleLocal(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/ LocalVariableInstruction inst) {
    if (inst instanceof IINC) {
      ...
      if (inst instanceof ALOAD) {
        ...
        throw new RuntimeException("Unknown load instruction type: " + inst);
      }
    } else if (inst instanceof StoreInstruction) {
      stack.pop();
    } else {
      throw new RuntimeException("Unknown local instruction type: " + inst);
    }
  }

  /*@RequiresNonNull("stack")*/
  private void handleReturn(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/ ReturnInstruction inst) {
    stack.clear();
  ...
  private void handleStack(
      /*>>>@UnknownInitialization @Raw TypeStack this,*/ StackInstruction inst) {
    if (inst instanceof DUP) {
      ...
      throw new RuntimeException("Unknown stack instruction type: " + inst);
    }
  }
  ...
  public static void testJavaClass(JavaClass clazz) {
    for (Method meth : clazz.getMethods()) {
      MethodGen mg =
          new MethodGen(
              meth, TypeStack.class.getName(), new ConstantPoolGen(clazz.getConstantPool()));
      ...
      if (mg.getInstructionList() != null) {
        ...
        if (meth.getReturnType() != Type.VOID) {
          assert stack.stack != null
              : "@AssumeAssertion(nullness): Checker Framework bug: does not infer flow facts from annotated types";
  ...
  public OperandStack getAfterInst(InstructionHandle inst) {
    ...
    if (s == null) throw new IllegalArgumentException("Could not find " + inst + " in the map");
  ...
  private static boolean sameType(Type t1, Type t2) {
    ...
    return both(Type.BOOLEAN, t1, t2)
        || both(Type.BYTE, t1, t2)
        || both(Type.CHAR, t1, t2)
        || both(Type.DOUBLE, t1, t2)
        || both(Type.FLOAT, t1, t2)
        || both(Type.INT, t1, t2)
        || both(Type.LONG, t1, t2)
  ...
  private static Type conv(Type t) {
    ...
    if (t == Type.BOOLEAN) return Type.INT;
    else if (t == Type.CHAR) return Type.INT;
    else if (t == Type.SHORT) return Type.INT;
    else if (t == Type.BYTE) return Type.INT;
    else return t;
  }
