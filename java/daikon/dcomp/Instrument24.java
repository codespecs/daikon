...
public class Instrument24 implements ClassFileTransformer {
  ...
  // Temporary  - remove method when DCInstrument24 is working.
  /*
  ...
  private void outputOldDebugFiles(JavaClass c, File directory, @BinaryName String className) {
    try {
      debug_transform.log("Dumping .oclass and .bcel for %s to %s%n", className, directory);
      // Write the byte array to a .oclass file.
      c.dump(new File(directory, c.getClassName() + ".oclass"));
      // write .bcel file
      BcelUtil.dump(c, directory);
      // move <name>.bcel to <name>.obecl!
    } catch (Throwable t) {
