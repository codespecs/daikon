...
public class SummaryInfo {
  ...
  public static class BadSummaryAnnotation extends RuntimeException {
    ...
    public BadSummaryAnnotation (String format, Object... args) {
  ...
  public SummaryInfo (java.lang.reflect.Method method, String annotation) {
    ...
    if (si.length != 2)
      throw new BadSummaryAnnotation ("method %s: annotation not in the form"
                         + " 'static|instance-fully_qualified_name'", method);
  ...
  public boolean original_exists() {
    ...
    try {
      ...
      Method original_method = c.getMethod (original_methodname,
                                            original_params());
      return true;
  ...
  public static void init() {
    ...
    for (java.lang.reflect.Method m : methods) {
      ...
      if (dfs == null)
        continue;
      if (verbose)
        System.out.printf ("Summary on method %s is %s%n", m, dfs.value());
      SummaryInfo si = new SummaryInfo (m, dfs.value());

      // Get the map for this class and create it if it is not there
      Map<MethodDef,String> class_map
        = jdk_method_map.get (si.original_classname);
      if (class_map == null) {
