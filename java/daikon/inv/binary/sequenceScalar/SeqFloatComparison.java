 ...
 **/
public final class SeqFloatComparison
  extends SequenceFloat
{
  ...
  public static SeqFloatComparison instantiate(PptSlice ppt, boolean seq_first) {
    ...
    if (debugSeqIntComparison) {
      System.out.println("SeqFloatComparison instantiated: "
                         + sclvar.name + " vs. " + seqvar.name);
    }
    ...
    if ((sclseq != null) && (SubSequenceFloat.isObviousSubSequence(seqvar, sclseq))) {
    ...
    return new SeqFloatComparison(ppt, seq_first, only_eq, obvious_le, obvious_ge);
  }
  ...
  {
    return core.isSameFormula(((SeqFloatComparison) other).core);
  }
    ...
    for (Iterator inv_itor = pptt.invariants_iterator(); inv_itor.hasNext(); ) {
      ...
      if (inv instanceof SeqFloatComparison) {
        ...
        if (isSameFormula(other)
            && (sclvar() == other.sclvar())
            && SubSequenceFloat.isObviousSubSequence (seqvar(), other.seqvar())) {
          return true;
        }
      }
    }
