...
public final class MathMDE {
  ...
  /** Return the greatest common divisor of the elements of integer array a. */
   ...
   * between the elements of integer array a.
   */
   ...
   * its iterator.
   */
  public static int[] modulus(Iterator itor) {
    if (!itor.hasNext())
      return null;
   ...
   * may be missing.
   */
  public static int[] modulus_strict(Iterator itor) {
    if (!itor.hasNext())
      return null;
  ...
  static final class MissingNumbersIterator implements Iterator {
    ...
    MissingNumbersIterator(int[] nums_) {
      nums = nums_;
      Arrays.sort(nums);
      current_index = 0;
    ...
    MissingNumbersIterator(Iterator nums_itor_) {
      nums_itor = nums_itor_;
	...
	if (nums != null) {
	  ...
	  if (! (prev_nonmissing < current_nonmissing))
	    throw new Error("Non-sorted Iterator supplied to MissingNumbersIterator: prev_nonmissing = " + prev_nonmissing + ", current_nonmissing = " + current_nonmissing);
	} else {
  ...
  public static int[] nonmodulus_strict(int[] nums) {
    ...
    return modulus_strict(new MissingNumbersIterator(nums));
  }
   ...
   * m) but all missing numbers in their range are.
   */
  public static int[] nonmodulus_strict(Iterator nums) {
    // Must not use regular modulus:  that can produce errors, eg
    // nonmodulus_strict({1,2,3,5,6,7,9,11}) => {0,2}.
    return modulus_strict(new MissingNumbersIterator(nums));
  }
  ...
  // missing_numbers that returns an integer array instead of an Iterator.
  // /**
  //  * Return a tuple of (r,m) where no number in NUMS is equal to r (mod
  //  * m) but all missing numbers in their range are.
  //  */
