...
public class TestNumbering {
    public static String toString(Object t) {
        if (t instanceof NumberedVarComputation) {
            NumberedVarComputation c = (NumberedVarComputation)t;
            
            return "Value computed at " + c.getOffset() + " (#" + c.getComputationNumber() + ")";
        } else if (t instanceof NumberedVarParam) {
            return "Parameter " + ((NumberedVarParam)t).getIndex();
        } else {
            return "???";
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (Enumeration e = c.getMethods(); e.hasMoreElements();) {
            ...
            if (m.getMethodName().equals(args[1])) {
                System.out.println("Varnumbers in " + m);
                ...
                for (int PC = 0; PC < starts.length; PC++) {
                    if (starts[PC]) {
                        ...
                        for (int i = 0; i < m.getData().getMaxStackWords(); i++) {
                            Object t = v.getStackNumber(PC, i);
                        ...
                        for (int i = 0; i < m.getData().getMaxLocalWords(); i++) {
                            Object t = v.getVariableNumber(PC, i);
