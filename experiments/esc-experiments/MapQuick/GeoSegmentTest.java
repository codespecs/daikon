...
public class GeoSegmentTest extends TestCase {
  ...
  public void testEquals() {
    ...
    assert("totally different objects are equal.", !gsNorth.equals(gsEast));
    assert("same points, different name are equal.", !gsEast2.equals(gsWest2));
    assert("same name, different points are equal.", !gsEast.equals(gsEast2));
    
  }
  
  public void testReverse() {
    assert("Reversed segment is not equal same segment reversed.",
           gsEast.reverse().equals(gsEast.reverse()));
    assert("Twice reversed segment is not same as initial.",
           gsEast.reverse().reverse().equals(gsEast));
    assert("New reversed item is not equal to its reversal.",
           gsWest.equals(gsWest2.reverse()));
    assert("New reversed item is not equal to its reversal.",
           gsWest.reverse().equals(gsWest2));
    assert("Segment equal to its reversal.",
           !gsEast.reverse().equals(gsEast));
    assert("Segment reversed twice is equal to its reversal.",
           !gsEast.reverse().reverse().equals(gsEast.reverse()));
    assert("Reversed segment reversed equals reversed.",
           !gsWest.reverse().equals(gsWest2.reverse()));
    // assert("Reversed zero segment doesn't equal itself.",
  ...
  public void testHeading() {
    ...
    assert("North heading (" + nh + ") is less than zero.", !(nh < 0.0));
    assert("North heading (" + nh + ") is greater or equal to 360.", !(nh >= 360.0));
    if (nh > tolerance) {
  ...
  public void testEquals2() {
    ...
    assert("Segment equality should use value equality, not reference equality",
	   gsNorth.equals(gsNorth2));

    assert("equals(non-GeoSegment) should be false",
	   !gsNorth2.equals("aString"));

    assert("equals(null) should be false",
  ...
  public void testHashCode() {
    ...
    assert(".equals() objects must have the same .hashCode()",
