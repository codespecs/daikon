  ...
  extends Route
{
  ...
  {
    super(r, gs);
    Assert.assert(r.end().equals(gs.p1()));
    Assert.assert(r.name().equals(gs.name()));
  }
  ...
  {
    Assert.assert((0 <= heading) && (heading < 360));
  ...
  {
    Assert.assert((0 <= from) && (from < 360));
    Assert.assert((0 <= to) && (to < 360));
  ...
  private static String roundToTenth(double n)
  {
    ...
    Assert.assert(Math.abs(Double.parseDouble(result) - n) <= 0.5);
    return result;
  }
