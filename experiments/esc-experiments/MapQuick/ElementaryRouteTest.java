...
public class ElementaryRouteTest extends TestCase {
  ...
  public void testImmutability() {
    ...
    assert("addSegment mutates receiver.", rReceiver.equals(rWest));
    assert("addSegment mutates argument.", gsArgument.equals(gsWest2));
    rReceiver.directions(0.0);
    assert("directions mutates receiver.", rReceiver.equals(rWest));
    rReceiver.hashCode();
    assert("hashCode mutates receiver.", rReceiver.equals(rWest));
    rReceiver.name();
    assert("name mutates receiver.", rReceiver.equals(rWest));
    rReceiver.elementaryRoutes();
    assert("elementaryRoutes mutates receiver.", rReceiver.equals(rWest));
    rReceiver.end();
    assert("end mutates receiver.", rReceiver.equals(rWest));
    rReceiver.endHeading();
    assert("endHeading mutates receiver.", rReceiver.equals(rWest));
    rReceiver.length();
    assert("length mutates receiver.", rReceiver.equals(rWest));
    rReceiver.start();
    assert("start mutates receiver.", rReceiver.equals(rWest));
    rReceiver.startHeading();
    assert("startHeading mutates receiver.", rReceiver.equals(rWest));
  }
  
  
  public void testName(){
    assert("the name rWest is not the same than the name of the segment gsWest it was created with",rWest.name().equals(gsWest.name()));
  }
  
  public void  testStart(){
    assert("the start point of rWest is not the same than the start point of the segment gsWest it was created with",rWest.start().equals(gpDowntown));
  }

  public void testEnd(){
    assert("the end point of rWest is not the same than the start point of the segment gsWest it was created with",rWest.end().equals(gpWest));
  }
  ...
  public void testEquals(){
    ...
    assert("rWest and rWestBis are created both from gsWest, but they are not equal",rWest.equals(rWestbis)); 
  }
  ...
  public void testAddSegment(){
    ...
    assert("the end point rWest.addSegment(gsWest2) is not the same end point than gsWest2",r.end().equals(gpWest2));
    assert("the start point rWest.addSegment(gsWest2) is not the same start point than rWest",r.start().equals(gpDowntown));
    ...
    ElementaryRoute rWestbis = new ElementaryRoute(gsWest);
    assert("after rWest.addSegment(gsWest2), rWest was mutated!!",rWest.equals(rWestbis));
  }
  ...
  public void testEquals2(){
    ...
    assert("r1 is created from (gpWest -> gpWest2) + (gpWest2 -> gpNorthWest2), it should not be equal to r2 created from (gpWest -> gpNorthWest2)",!rDW2.equals(rDWW2)); 
  }
  ...
  public void testElementaryRoutes(){
    ...
    else assert("the array of ElementaryRoutes of the ElementaryRoute r doesn't contain r",era[0].equals(rWest));
  }
