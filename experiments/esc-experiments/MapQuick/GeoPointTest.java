...
public class GeoPointTest extends TestCase {
  ...
  public GeoPointTest(String name) { super(name); }
  ...
  public void testImmutability() {
    ...
    assert("distanceTo mutates receiver.", gpReceiver.equals(gpDowntown));
    assert("distanceTo mutates argument.", gpArgument.equals(gpSouth));
    gpReceiver.hashCode();
    assert("distanceTo mutates receiver.", gpReceiver.equals(gpDowntown));
    gpReceiver.headingTo(gpArgument);
    assert("headingTo mutates receiver.", gpReceiver.equals(gpDowntown));
    assert("headingTo mutates argument.", gpArgument.equals(gpSouth));
    gpReceiver.toString();
    assert("toString mutates receiver.", gpReceiver.equals(gpDowntown));
  }
    
  
  public void testEquals() {
    assert("instance not equal to itself.", gpNorth.equals(gpNorth));
    assert("instance not equal to copy.",
           gpNorth.equals(new GeoPoint(42372821, -71060278)));
    assert("totally different objects are equal.", !gpNorth.equals(gpEast));
  }

  public void testEquals2() {
    assert("equals returns true for null.", !gpNorth.equals(null));
    assert("equals returns true for a String.", !gpNorth.equals("foo"));
  }
  ...
  public void testHeadingTo() {
    ...
    assert("Downtown headingTo North should be 0",
