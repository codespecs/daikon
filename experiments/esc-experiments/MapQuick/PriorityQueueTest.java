  ...
  extends TestCase
{
  ...
  public PriorityQueueTest(String name) { super(name); }
  ...
  private void assertContainsMatched(PriorityQueue q, Object[] o, boolean[] contains)
  {
    for (int i = o.length; i < o.length; i++) {
      assert(q.contains(o[i]) == contains[i]);
    }
  }
  ...
  public void testContains()
  {
    ...
    insert(q, d[0]); in[0] = true;
    assertContainsMatched(q, d, in);
    insert(q, d[2]); in[2] = true;
    assertContainsMatched(q, d, in);
    insert(q, d[4]); in[4] = true;
    assertContainsMatched(q, d, in);
    q.extractMin();  in[0] = false;
    assertContainsMatched(q, d, in);
    q.extractMin();  in[2] = false;
    assertContainsMatched(q, d, in);
    q.extractMin();  in[4] = false;
    assertContainsMatched(q, d, in);
  }
  ...
  public void testExtract()
  {
    ...
    insert(q, d[4]); in[4] = true;
    insert(q, d[2]); in[2] = true;
    insert(q, d[3]); in[3] = true;
    insert(q, d[1]); in[1] = true;
    assertContainsMatched(q, d, in);
    assertEquals(q.extractMin(), d[0]); in[0] = false;
    assertContainsMatched(q, d, in);
    assertEquals(q.extractMin(), d[1]); in[1] = false;
    assertContainsMatched(q, d, in);
    assertEquals(q.extractMin(), d[2]); in[2] = false;
    assertContainsMatched(q, d, in);
    assertEquals(q.extractMin(), d[3]); in[3] = false;
    assertContainsMatched(q, d, in);
    assertEquals(q.extractMin(), d[4]); in[4] = false;
    assertContainsMatched(q, d, in);
    try {
      q.extractMin();
