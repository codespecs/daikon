 ...
 * TODO
 */
public class CompositeRoutePath
  implements Path
{
  ...
  {
    this(node, new HashSet(goals), null);
  }
  ...
  private CompositeRoutePath(GeoSegment node, Set goals, CompositeRoutePath path)
  {
    ...
    Assert.assert(goals != null);
    Assert.assert(goals.size() >= 1);
    Assert.assert((path == null) || (path.node.p2().equals(node.p1())));
  ...
  public void checkRep()
  {
    ...
    Assert.assert(goals.size() >= 1);
    for (Iterator iter = goals.iterator(); iter.hasNext(); ) {
      Assert.assert(iter.next() instanceof GeoSegment);
    }

    // cost is increasing
    if (path != null) {
      Assert.assert(astar_cost >= path.astar_cost);
      Assert.assert(route_cost >= path.route_cost);
    }
    ...
    while (elements.hasNext()) {
      ...
      Assert.assert(prev.p2().equals(cur.p1()));
      prev = cur;
    }
  }
  ...
  {
    Assert.assert(o instanceof GeoSegment);
