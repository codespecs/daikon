...
public class StreetNumberSet
{
  ...
  public StreetNumberSet(String numbers)
  {
	...
	low = high = Integer.parseInt(range);
      ...
      Assert.assert(low >= 0);
      Assert.assert(high >= 0);
      Assert.assert(low <= high);
      Assert.assert(parityOf(low) == parityOf(high));
  ...
  {
    // parallel arrays
    Assert.assert(begins != null);
    Assert.assert(ends != null);
    Assert.assert(begins.length == ends.length);
    ...
    for (int i = 0; i < begins.length; i++) {
      ...
      Assert.assert(b <= e);
      Assert.assert(parityOf(b) == parityOf(e));
  ...
  public int orderStatistic(int n)
  {
    ...
    Assert.assert((0 <= result) && (result <= size()));
    return result;
  }
