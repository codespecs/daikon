...
public class RatPolyStackTest extends TestCase {
	...
	for(int i=0; i<desc.length(); i++) {
	    ...
	    switch(c) {
	    case '0':
		assert( asstr+zero.unparse(), eqv( p, zero ));
		continue polys;
	    case '1':
		assert( asstr+one.unparse(), eqv( p, one ));
		continue polys;
	    case '2':
		assert( asstr+two.unparse(), eqv( p, two ));
		continue polys;
	    case '3':
		assert( asstr+three.unparse(), eqv( p, three ));
		continue polys;
	    case '4':
		assert( asstr+four.unparse(), eqv( p, four ));
		continue polys;
	    case '5':
		assert( asstr+five.unparse(), eqv( p, five ));
		continue polys;
	    case '6':
		assert( asstr+six.unparse(), eqv( p, six ));
		continue polys;
	    case '7':
		assert( asstr+seven.unparse(), eqv( p, seven ));
		continue polys;
	    case '8':
		assert( asstr+eight.unparse(), eqv( p, eight ));
		continue polys;
	    case '9':
		assert( asstr+nine.unparse(), eqv( p, nine ));
		continue polys;
	    default: 
	...
	assertStkIs( "223" );
	assert( stk1.size() == 3 );
	assert( eqv( stk1.get(0), two ));
	assert( eqv( stk1.get(1), two ));
	assert( eqv( stk1.get(2), three ));
