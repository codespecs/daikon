...
public class RatNumTest extends TestCase {
	...
	for (int i=0; i<ratnums.length; i++) {
	    assert(ratnums[i].equals(ratnums[i]));
	}
    }
    public void testEquals() {
	assert( one.equals(one) );
	assert( one.add(one).equals(two) );
	assert( one_I_two.equals( one.div(two) ) );
	assert( three_I_two.equals( three.div(two) ) );

	assert( one_I_zero.equals( one_I_zero ) );
	assert( one_I_zero.equals( negOne_I_zero ) );
	assert( one_I_zero.equals( hundred_I_zero ) );

	assert( ! one.equals(zero) );
	assert( ! zero.equals(one) );
	assert( ! one.equals(two) );
	assert( ! two.equals(one) );
	assert( ! one.equals(negOne) );
	assert( ! negOne.equals(one) );

	assert( ! one.equals(one_I_two) );
	assert( ! one_I_two.equals(one) );
	assert( ! one.equals(three_I_two) );
	assert( ! three_I_two.equals(one) );
    }
    
    private void assertGreater( RatNum larger, RatNum smaller ) {
	assert( larger.compareTo(smaller) > 0 );
	assert( smaller.compareTo(larger) < 0 );
    }
    public void testCompareToReflexive() {
	// reflexivitiy
	for(int i=0; i<ratnums.length; i++) {
	    assert( ratnums[i].compareTo(ratnums[i]) == 0);
	}
    }
	...
	for(int i=0; i<ratnans.length; i++) {
	    for(int j=0; j<ratnans.length; j++) {	    
		assert( ratnans[i].compareTo(ratnans[j]) == 0);
	    }
	...
	for (int i=0; i<ratnans.length; i++) {
	    assert(ratnans[i].isNaN());
	}
	for (int i=0; i<ratnanans.length; i++) {
	    assert(!ratnanans[i].isNaN());
	}
    }
