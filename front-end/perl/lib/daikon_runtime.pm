...
sub set_output_style {
    my (@styles) = @_;
    my $i = 0;
    for my $kind ('types', 'decls', 'dtrace') {
...
#    warn "Finishing $package";
    if (not exists $output_style{dtrace}{combined_fh}) {
        close $filehandles{$package};
    }
    delete $filehandles{$package};

    if ($tracing and keys %{$decls{$package}}) {
        my ($fh, $close_when_done) = open_for_output($package, "decls");
        print $fh "// Declarations for the Perl package $package\n";
        print $fh "// Generated by daikon_runtime.pm\n\n";
        print $fh "VarComparability\nnone\n\n";
        for my $lines_ref (values %{$decls{$package}}) {
            print $fh join("", @$lines_ref), "\n";
        }
        close $fh if $close_when_done;
    }

    if (keys %{$types{$package}}) {
